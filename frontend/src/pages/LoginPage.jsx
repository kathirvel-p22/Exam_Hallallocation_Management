...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && user) navigate(ROLE_REDIRECT[user.role] || '/dashboard', { replace: true });
  }, [isAuthenticated]);

  // Pre-fill demo credentials if arriving from portal card
  useEffect(() => {
    if (hintRole && ROLE_DEMOS[hintRole]) {
  ...
  const onSubmit = async ({ email, password }) => {
    ...
    try {
      ...
      toast.success(`Welcome back, ${result.user.student?.name || result.user.invigilator?.name || email.split('@')[0]}!`);
    ...
    <div className="min-h-screen flex bg-[#F9F6EE]">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[420px]">
          ...
          <h3 className="font-display text-[32px] font-bold text-navy mb-1">Welcome back</h3>
          <p className="text-[14px] text-navy/45 mb-7">Sign in to your portal to continue</p>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[11px] text-navy/30 font-mono">or enter credentials</span>
            <div className="flex-1 h-px bg-navy/10" />
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                ...
                <button type="button" className="text-[11px] text-gold hover:text-[#A8880A] font-semibold">Forgot password?</button>
              </div>
              <input
                type="password"
            ...
            <button type="submit" disabled={isLoading} className="btn btn-gold btn-lg w-full justify-center mt-2">
              {isLoading ? <><Spinner size="sm" color="navy" /> Signing in…</> : 'Sign In →'}
            </button>
          </form>

          <p className="text-center text-[12px] text-navy/35 mt-7 leading-relaxed">
            Access is restricted to authorized institutional users.<br />
            Contact your exam administrator for an account.
          </p>
